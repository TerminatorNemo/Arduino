...
Отсчет до взрыва или до ввода пароля либо мастер пароля
2 режим
Ввод премени на игру
а потом кто дольше продержиться игра до времени, нажатие на 5 выход из режима
3 решим
время на удержание
ввод времени удержания
игра до момента пока команда не будет удерживать точку на протяжении всего времени, нажатие на 5 выход из режима
*/
...
void loop() {
  ...
  if (but_in == 1) {
    del();
    disp2.displayByte (_t, _i, _n, _E);
    Serial.println("boom");
    boom();
  }
  if (but_in == 2) {
    del();
    disp2.displayByte (_t, _i, _n, _n);
    Serial.println("time_to_win");
    time_to_win();
  }
  if (but_in == 3) {
    del();
    disp2.displayByte (_t, _i, _n, _E);
    Serial.println("time_of_retention");
    time_of_retention();
  }
  disp1.point(0);
}
...
void time_of_retention() {
  ...
  while (1) {
    ...
    long miliss_new_pisk = millis()/150;

    if (flag2 == 1) {
      time1 = millis() / 1000 - start_time;
      print_time1(disp1, time1_save - time1);
      Serial.print("time2_save - time1  ");
      Serial.println(time2_save - time1);
      if (time2_save - time1 > 60) {
          if(millis()%800>20&&millis()%800<700){digitalWrite(tonePin,LOW);}else{digitalWrite(tonePin,HIGH);}
      } else {
          if(millis()*2%800>20&&millis()*2%800<700){digitalWrite(tonePin,LOW);}else{digitalWrite(tonePin,HIGH);}
      }


        /*
      if (flag1 != millis() / 1000 % 2) {
        if (time2_save - time2 < 60) {
          tone(tonePin, 0, 150);
        } else {
          tone(tonePin, 2, 500);
        }flag1 = millis() / 1000 % 2;
      ...
      */
      ...
      if (time2_save - time2 > 60) {
          if(millis()%1000>20&&millis()%1000<800){digitalWrite(tonePin,LOW);}else{digitalWrite(tonePin,HIGH);}
      } else {
          if(millis()*2%800>20&&millis()*2%800<700){digitalWrite(tonePin,LOW);}else{digitalWrite(tonePin,HIGH);}
      }
      /*
      if (flag1 != millis() / 1000 % 2) {
        if (time2_save - time2 < 60) {
          tone(tonePin, frec, 150);
        } else {
          tone(tonePin, frec, 150);
        } flag1 = millis() / 1000 % 2;
      }
      */
      if (time2_save - time2 <= 0) {
        win();
        break;
      }
    }
...
void boom() {
  ...
  while (time2 / 1000 < time_of_game) {
    ...
    if (time_to_boom < time_of_game - time2 / 1000) {
      //tone(tonePin, frec, 150);
      if(millis()%800>20&&millis()%800<700){digitalWrite(tonePin,LOW);}else{digitalWrite(tonePin,HIGH);}
      //Serial.println("1");
      flag = millis() / 1000 % 2;
    }
    else if ( time_to_boom >= time_of_game - time2 / 1000) {
      //tone(tonePin, frec, 150);
          if(millis()*2%800>20&&millis()*2%800<700){digitalWrite(tonePin,LOW);}else{digitalWrite(tonePin,HIGH);}
     // Serial.println("2");
      flag = millis() / 500 % 2;
  ...
  tone(tonePin, frec, 1000); delay(1000);
  NeObezv();
}
