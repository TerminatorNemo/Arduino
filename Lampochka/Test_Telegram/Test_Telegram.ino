...
void setup(void){
 ...
 // char temp[11]=WiFi.localIP()
  ...
  server.on("/hr00", [](){                                        // При HTTP запросе вида http://192.168.4.1/relay_switch
      server.send(200, "text/plain", hr00());                     // Отдаём клиенту код успешной обработки запроса, сообщаем, что формат ответа текстовый и возвращаем результат выполнения функции relay_switch 
  }); 
  ...
  bot.sendMessage("629436257",String(IP[0])+"."+String(IP[1])+"."+String(IP[2])+"."+String(IP[3])+":80");
...
void loop(void){
  ...
  ftpSrv.handleFTP();
  if(hr==0){
    bot.sendMessage("629436257","hr=0");
    hr=8;
  }else if (hr==1){
    bot.sendMessage("629436257","hr=1");
    hr=8;
  }
//test_bot__test_bot__test_bot__test_bot__test_bot__test_bot__test_bot__test_bot__test_bot__test_bot__
  if (millis() - bot_lasttime > BOT_MTBS)
  {
    ...
    while (numNewMessages)
    {
     ...
     //не сработало  bot.sendMessage("629436257","Ia poluchil tvoio sms");
    }
