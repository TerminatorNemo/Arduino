...
const char *ssid =  "Kyivstar_F0E6";  // Имя вайфай точки доступа
const char *pass =  "48544854"; // Пароль от точки доступа

const char *mqtt_server = "192.168.1.16"; // Имя сервера MQTT
const int mqtt_port = 1883; // Порт для подключения к серверу MQTT
const char *mqtt_user = "Nemo_home"; // Логин от сервер
const char *mqtt_pass = "48544854"; // Пароль от сервера
    ...
    if (WiFi.status() == WL_CONNECTED) {
        if (!client.connected()) {
            ...
            if (client.connect(MQTT::Connect("arduinoClient2")
                ...
                client.subscribe("Lamp");                  // подписывааемся по топик с данными для светодиода
