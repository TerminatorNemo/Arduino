...
float xxx = 1;
int m =996;//красная 996
float perezapolenie = 0;
int x =1;//_____________________________ОТЛАДКА
...
int b4 = 0;
void setup() {
if(test_0==0)perezapolenie = EEPROM.read(60);
EEPROM.update(60,perezapolenie);

if(x==1)  Serial.begin(250000);
...
//__________________ОТЛАДКА
 ...
 Serial.print("  "); 
 Serial.print(t);
 Serial.print("   сколько было перезаполенеий   "); 
 Serial.print(perezapolenie);
 Serial.print("   При каком значении   "); 
 Serial.println(tinf);
 }       
...
//________________________________________________________________________________________________________________Вывод
void Vivod(){
if(chas<0){
  chas=chas+24;
  }
